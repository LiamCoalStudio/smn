...
inline ifile open_in(const str &name) { return ifile(name); }

inline ofile open_out(const str &name) { return ofile(name); }

inline iofile open_io(const str &name) { return iofile(name); }

template<typename T>
inline void write(ofile &out, T t) { out << t; }
...
template<class T>
inline void close(T &stream) { stream.close(); }

template<class T>
inline void flush(T &stream) { stream.flush(); }
...
template<typename T>
void println(const T &text) { std::cout << text << std::endl; }

template<typename T>
void print(const T &text) { std::cout << text; }
